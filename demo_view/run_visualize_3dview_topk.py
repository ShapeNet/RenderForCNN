...
if __name__ == '__main__':
    ...
    model_obj_file = os.path.join(g_render4cnn_root_folder, 'demo_render', 'sample_model2', 'model.obj')
    original_image_file = os.path.join(BASE_DIR, 'aeroplane_image.jpg')
    ...
    for k in range(topk):
        ...
        model_obj_file, str(a), str(e), str(t), str(2.0), rendered_image_file_prefix+str(k)+'.png')
    ...
    for k in range(topk):
        ...
        draw.text((0,0), 'rank: %d, confidence: %f\nazimuth=%f, elevation=%f, tilt=%f' % (k+1, v[4*k+3], v[4*k], v[4*k+1], v[4*k+2]), (0,255,0))
        im2s.append(im2)
